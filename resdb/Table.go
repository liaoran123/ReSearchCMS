...
type Table struct {
	...
	*/
	...
	primary  string     // 主键字段，只支持单个主键
	counter  AutoInt    // 自动增值计数器，使用自定义的AutoInt
	index    [][]string // 索引数组，支持单索引和组合索引
...
// MarshalJSON 实现json.Marshaler接口，自定义序列化逻辑
	...
	func (t *Table) MarshalJSON() ([]byte, error) {
		...
		temp := struct {
			...
			Counter  int            `json:"counter"` // 将AutoInt转换为int
			Index    [][]string     `json:"index"`
			FullText []string       `json:"full_text"`
			FtLen    uint8          `json:"ft_len"`
		}{
			Name:     t.name,
			Fields:   t.fields,
			Primary:  t.primary,
			Counter:  t.counter.Get(), // 获取AutoInt的int值
			Index:    t.index,
			FullText: t.fullText,
			FtLen:    t.ftlen,
		}
...
// UnmarshalJSON 实现json.Unmarshaler接口，自定义反序列化逻辑
	...
	func (t *Table) UnmarshalJSON(data []byte) error {
		...
		var temp struct {
			...
			Fields   map[string]any `json:"fields"`
			Primary  string         `json:"primary"`
			Counter  int            `json:"counter"` // 将int转换为AutoInt
			Index    [][]string     `json:"index"`
			FullText []string       `json:"full_text"`
			FtLen    uint8          `json:"ft_len"`
		}
		...
		// 将临时结构体的值复制到Table实例
		t.name = temp.Name
		t.fields = temp.Fields
		t.primary = temp.Primary
		t.counter.Set(temp.Counter) // 设置AutoInt的值
		t.index = temp.Index
		t.fullText = temp.FullText
		t.ftlen = temp.FtLen

		// 保持rsdb不变，不需要从JSON中恢复
...
func (t *Table) GetIndexsPrefix(fieldsBytes *map[string][]byte, matchesfield ...string) [][]byte {
...
func (t *Table) GetFullTextsPrefix(fieldsBytes *map[string][]byte, matchesfield ...string) [][]byte {
...
func (t *Table) IndexKV(fieldsBytes *map[string][]byte, batch *leveldb.Batch, put bool, ufield ...string) {
	idxs := t.GetIndexsPrefix(fieldsBytes, ufield...)
...
// put全文索引KV
// 匹配字段ufield ，用于记录修改某个字段时，只需要更新该字段的索引即可。
func (t *Table) FullTextKV(fieldsBytes *map[string][]byte, batch *leveldb.Batch, put bool, ufield ...string) {
	idxs := t.GetFullTextsPrefix(fieldsBytes, ufield...)
	value := (*fieldsBytes)[t.primary]
	if len(value) == 0 {
		fmt.Printf("主键 '%s' 的值为空，无法创建索引\n", t.primary)
	}
...
func (t *Table) Search(fields *map[string]any) iterator.Iterator {
	if t.fields == nil {
		return nil
	}
	var field []string
