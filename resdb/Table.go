...
func (t *Table) MatchIndex(indexs []string) *Indexs {
	...
	isPrimary := indexCount == len(t.primary)
	if isPrimary {
...
func (t *Table) GetFieldJoin(fieldsBytes *map[string][]byte, fields []string) (r []byte) {
	var exists bool
	var fieldValue []byte
	var isnil bool
...
func (t *Table) IndexsKV(fieldsBytes *map[string][]byte, batch *leveldb.Batch, put bool, ufield ...string) {
	...
	//value := (*fieldsBytes)[t.primary[0]]
	value := t.GetFieldJoin(fieldsBytes, t.primary)
	if len(value) == 0 {
